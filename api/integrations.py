...
def authorize_zoho(auth=_AUTHORIZED):
    ...
    auth_url = f"{accounts_url}/oauth/v2/auth?scope={scope}&client_id={client_id}&response_type=code&access_type=offline&redirect_uri={redirect_uri}"
...
def zoho_callback(code: str):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO integrations (service_name, access_token, refresh_token, token_expiry)
                VALUES ('zoho_crm', %s, %s, %s)
...
def get_zoho_status(auth=_AUTHORIZED):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT last_sync, created_at FROM integrations WHERE service_name = 'zoho_crm'")
            record = cur.fetchone()
            if record:
                return {
                    "connected": True,
...
def sync_zoho_leads(auth=_AUTHORIZED):
    """Manually trigger lead sync from Zoho"""
    leads = zoho_service.fetch_leads()
    if not leads:
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("UPDATE integrations SET last_sync = CURRENT_TIMESTAMP WHERE service_name = 'zoho_crm'")
            conn.commit()
    finally:
        conn.close()
