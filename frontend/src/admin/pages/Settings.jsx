...
const Settings = () => {
   ...
   const savePrompt = async (key, value) => {
      try {
         setSavingKey(key);
         await api.post(`/api/admin/config/${key}`, { value });
         setSaveSuccess(key);
                     ...
                     <div key={prompt.key} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                        <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                           ...
                           <button
                              ...
                              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-lg ${saveSuccess === prompt.key
